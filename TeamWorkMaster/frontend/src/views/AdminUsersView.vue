  ...
  <div class="space-y-6 pb-6">
    ...
    <div class="bg-slate-800 p-6 rounded-xl shadow-sm border border-slate-700 flex flex-col md:flex-row md:justify-between md:items-center gap-4">
      ...
      <div class="flex flex-col md:flex-row items-center space-y-3 md:space-y-0 md:space-x-4 w-full md:w-auto">
        ...
        <div class="relative w-full md:w-64">
          ...
          <input v-model="searchQuery" type="text" placeholder="Tìm username, họ tên, email..." 
    ...
    <div class="bg-slate-800 rounded-xl shadow-sm border border-slate-700 flex flex-col">
      <div class="overflow-x-auto relative">
        <table class="w-full text-left border-collapse whitespace-nowrap">
          <thead>
            <tr class="bg-slate-900 text-slate-400 text-xs uppercase tracking-wider border-b border-slate-700">
              ...
              <th class="py-4 px-4 font-semibold w-40">Ngày tạo</th> 
              <th class="py-4 px-4 font-semibold text-center w-32">Trạng thái</th>
              <th class="py-4 px-4 font-semibold text-right w-36">Thao tác</th>
            </tr>
          </thead>
          <transition-group name="list" tag="tbody" class="divide-y divide-slate-700/50 text-slate-300 text-sm">
            <tr v-for="(user, index) in paginatedUsers" :key="user.id" class="hover:bg-slate-750/50 transition-colors" :class="!user.isActive ? 'opacity-60 bg-slate-800/30' : ''">
              ...
              <td class="py-4 px-4 text-xs text-slate-400 font-medium">{{ formatDateTime(user.createdAt) }}</td>
              ...
              <td class="py-4 px-4 text-right space-x-1">
                ...
                <button @click="openLockModal(user)" class="p-2 rounded-lg transition-all" 
        ...
        <div v-if="paginatedUsers.length === 0" class="p-8 text-center text-slate-500 italic border-t border-slate-700/50">
      ...
      <div v-if="totalPages > 1" class="p-4 border-t border-slate-700 flex justify-between items-center bg-slate-900/50 rounded-b-xl">
        <span class="text-sm text-slate-400">Trang <span class="font-bold text-white">{{ currentPage }}</span> / {{ totalPages }}</span>
        <div class="flex space-x-2">
          ...
          <button v-for="page in totalPages" :key="page" @click="currentPage = page" class="w-8 h-8 rounded-lg text-sm font-bold transition-colors flex items-center justify-center" :class="currentPage === page ? 'bg-blue-600 text-white shadow-lg shadow-blue-500/30' : 'bg-slate-700 text-slate-300 hover:bg-slate-600'">{{ page }}</button>
    ...
    <ConfirmLockModal v-if="isLockModalOpen" :user="selectedUser" @close="isLockModalOpen = false" @confirm="executeToggleLock" />
...
import ConfirmLockModal from '../components/ConfirmLockModal.vue' // Import Modal Khóa
...
const isLockModalOpen = ref(false) // State quản lý Modal Khóa
...
const filteredUsers = computed(() => {
  ...
  else if (statusFilter.value === 'locked') result = result.filter(user => !user.isActive);
...
// Mở Modal Khóa/Mở khóa thay vì dùng Alert
...
const handleDeleteUser = async (userId) => {
  try {
    ...
    if (data.success) {
      ...
      addToast('Tài khoản đã được xóa vĩnh viễn!', 'success');
      loadUsers(); 
    } else { 
      addToast("Xóa thất bại: " + (data.message || 'Lỗi không xác định'), 'error'); 
    }
  } catch (error) { addToast("Lỗi kết nối Server Java!", 'error'); }
}

// Thực thi API khi người dùng bấm Đồng ý trong ConfirmLockModal
const executeToggleLock = async (user) => {
  ...
  try {
    ...
    if (data.success) {
      ...
      addToast(`Đã ${actionName.toLowerCase()} tài khoản thành công!`, 'success');
      loadUsers(); 
    } else {
      isLockModalOpen.value = false;
      addToast("Lỗi: " + data.message, 'error');
    }
  } catch (error) {
    isLockModalOpen.value = false;
